...
export default function MonthlyCategoryStats() {
  ...
  const { categoryStats, maxDuration } = useMemo(() => {
    ...
    for (const day of monthlyData) {
      const dayLabel = String(new Date(day.date).getDate()); // 1~31일
              ...
              <BarChart data={cat.dailyTrend}>
                <XAxis
                  ...
                  tickFormatter={(day: string) => {
                    ...
                    return trend?.count ? `${day}일 (${trend.count})` : `${day}일`;
                  }}
                />
